...
const FloatingAccessibilityBar = () => {
  ...
  return (
    <>
      ...
      {(showFloating || window.innerWidth >= 768) && (
        <div className="fixed flex flex-col gap-3 top-1/2 -translate-y-1/2 right-6 z-[9999]">
          {buttons.map((btn, i) => (
            <a
              key={i}
              id={btn.id}
      ...
      {activeModal && (
        <div
          className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 sm:p-6 z-[9998] backdrop-blur-sm"
          onClick={() => setActiveModal(null)}
        >
          <div
            className="w-full max-w-md sm:max-w-lg max-h-[90vh] bg-white shadow-2xl rounded-2xl flex flex-col overflow-hidden transform transition-all duration-300 scale-100 hover:scale-[1.01]"
            ...
            <div className="flex-1 p-5 space-y-5 overflow-y-auto bg-gray-50">
              {activeModal === "penilaian" && (
                <form className="space-y-6" onSubmit={handleSubmit}>
                  <div>
                    <label className="block mb-3 font-semibold text-gray-800">😊 Tingkat Kepuasan Anda</label>
                    <div className="flex justify-around p-3 bg-white rounded-lg shadow">
                      {[ "sangat_puas", "puas", "cukup", "tidak_puas"].map((val, idx) => (
                        <label key={idx} className="flex flex-col items-center transition-transform cursor-pointer hover:scale-110">
                          <input
                            type="radio"
                            name="kepuasan"
                            value={val}
                          ...
                          <span className="mb-1 text-3xl transition-transform peer-checked:scale-125 peer-checked:text-cyan-800">
                            {val === "sangat_puas" ? "😄" : val === "puas" ? "🙂" : val === "cukup" ? "😐" : "🙁"}
                          </span>
                          <span className="text-xs text-gray-600">
                            {val === "sangat_puas" ? "Sangat Puas" : val === "puas" ? "Puas" : val === "cukup" ? "Cukup" : "Tidak Puas"}
                          </span>
                        </label>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="block mb-3 font-semibold text-gray-800">
                      📌 Apakah Anda dapat menemukan berita/informasi yang dicari?
                    </label>
              ...
              {activeModal === "aksesCepat" && (
                <div className="space-y-4">
                  <p className="leading-relaxed text-gray-700">Dapatkan akses cepat ke layanan publik Diskominfo Kota Bogor.</p>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {["/layanan","/agenda","/kontak","/faq"].map((href, idx) => (
                      <a
                        ...
                        onClick={() => setActiveModal(null)}
                      >
                        {href === "/layanan" ? "Layanan Publik" : href === "/agenda" ? "Agenda Kegiatan" : href === "/kontak" ? "Kontak Instansi" : "FAQ"}
                      </a>
                    ))}
                  </div>
                </div>
